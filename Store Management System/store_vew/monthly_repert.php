...
<body class="bg-light">
        <div class="container py-5">
                ...
                <h4 class="mb-4 text-center">
                        <?php
                        // Display current month and year, e.g. May 2025
                        echo date("F Y");
                        ?>
                </h4>

                <!-- 1. New Products Added This Month -->
                <div class="card mb-4">
                        <div class="card-header bg-success text-white">
                                <h4 class="mb-0">New Products Added This Month</h4>
                        </div>
                        <div class="card-body">
                                <?php
                                $sql1 = "SELECT * FROM product WHERE YEAR(product_entry_date) = YEAR(CURDATE()) AND MONTH(product_entry_date) = MONTH(CURDATE())";
                            ...
                            <thead class='table-light'>
                                ...
                                <td>{$row['selling_price']}</td>
                                        ...
                                        echo "<p class='text-muted'>No new products added this month.</p>";
                                }
                                ?>
                        </div>
                </div>

                <!-- 2. Updated/Restocked Products This Month -->
                <div class="card mb-4">
                        <div class="card-header bg-info text-white">
                                <h4 class="mb-0">Updated/Restocked Products This Month</h4>
                        </div>
                        <div class="card-body">
                                <?php
                                $sql2 = "SELECT * FROM report WHERE YEAR(adding_date) = YEAR(CURDATE()) AND MONTH(adding_date) = MONTH(CURDATE())";
                            ...
                            <thead class='table-light'>
                                ...
                                <td>{$selling_price}</td>
                                        ...
                                        echo "<p class='text-muted'>No products updated this month.</p>";
                                }
                                ?>
                        </div>
                </div>

                <!-- 3. Sold Products This Month -->
                <div class="card mb-4">
                        <div class="card-header bg-danger text-white">
                                <h4 class="mb-0">Sold Products This Month</h4>
                        </div>
                        <div class="card-body">
                                <?php
                                $sql3 = "SELECT * FROM profit WHERE YEAR(sell_date) = YEAR(CURDATE()) AND MONTH(sell_date) = MONTH(CURDATE())";
                                ...
                                <tr>
                                    ...
                                    <th>Profit</th>
                                        ...
                                        while ($row = $result_profit->fetch_assoc()) {
                                                $product_name = 'Unknown';
                                                $sql_product = "SELECT product_name FROM product WHERE product_id = {$row['product_id']}";
                                                $product_result = $conn->query($sql_product);
                                                if ($product_result->num_rows > 0) {
                                                        $product_row = $product_result->fetch_assoc();
                                                        $product_name = $product_row['product_name'];
                                                ...
                                                $total_profit += $row['profit_item'];

                                                echo "<tr>
                                <td>$counter</td>
                                <td>{$product_name}</td>
                                <td>{$row['quantity']}</td>
                                <td>{$row['profit_item']}</td>
                              </tr>";
                                                $counter++;
                                        }
                                        echo "</tbody></table></div>";
                                        echo "<p class='fw-bold'>Total Profit This Month: {$total_profit}TK</p>";
                                } else {
                                        echo "<p class='text-muted'>No products sold this month.</p>";
                                }
                                ?>
                        </div>
                </div>
